...
class LegalResearchApp {
    ...
    setupSocketListeners() {
        ...
        this.socket.on('crawling_status', (data) => {
            this.updateCrawlingStatus(data);
        });
    ...
    updateProgress(data) {
        ...
        // Show/hide crawling status section based on current step
        const crawlingSection = document.getElementById('crawling-status-section');
        if (data.current_step && data.current_step.toLowerCase().includes('searching')) {
            crawlingSection.classList.remove('hidden');
        }
    }

    updateCrawlingStatus(data) {
        const crawlingMessages = document.getElementById('crawling-messages');
        const crawlingSection = document.getElementById('crawling-status-section');
        
        // Show the crawling section
        crawlingSection.classList.remove('hidden');
        
        // Create new message element
        const messageElement = document.createElement('div');
        messageElement.className = 'flex items-start space-x-2 text-sm p-2 bg-white rounded border-l-4 border-blue-500 fade-in';
        
        // Parse message for different types of updates
        ...
        if (message.includes('✅')) {
            ...
            iconClass = 'fas fa-search text-blue-500';
        } else if (message.includes('🌐')) {
            iconClass = 'fas fa-globe text-indigo-500';
        } else if (message.includes('📄')) {
        ...
        crawlingMessages.appendChild(messageElement);
        
        // Keep only last 10 messages for performance
        while (crawlingMessages.children.length > 10) {
            crawlingMessages.removeChild(crawlingMessages.firstChild);
        }
        
        // Auto-scroll to bottom
        crawlingMessages.scrollTop = crawlingMessages.scrollHeight;
    }

    formatCrawlingDetails(details) {
        const formatters = [];
        
        if (details.source_count) {
            formatters.push(`Found: ${details.source_count} sources`);
        }
        if (details.domains) {
            formatters.push(`Domains: ${details.domains.join(', ')}`);
        }
        if (details.progress) {
            formatters.push(`Progress: ${details.progress}`);
        }
        if (details.duration) {
            formatters.push(`Duration: ${details.duration}`);
        }
        if (details.legal_sources !== undefined && details.news_sources !== undefined) {
            formatters.push(`Legal: ${details.legal_sources}, News: ${details.news_sources}`);
        }
